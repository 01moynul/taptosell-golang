...
// 1. It locks the cart and products.
// 2. It calculates the total price.
// 3. It checks the user's wallet balance.
// 4. It creates the order (status 'processing' or 'on-hold').
// 5. It deducts funds from the wallet (if processing).
// 6. It deducts stock from the products (if processing).
// 7. It snapshots the items into 'order_items'.
// 8. It clears the cart.
// ALL of this happens in a single, atomic database transaction.
func (h *Handlers) Checkout(c *gin.Context) {
	...
	// 2. --- Begin Transaction ---
	// We set a high isolation level to prevent "dirty reads"
	// and ensure our stock/price check is accurate.
	...
	// to ensure stock/price can't be changed by another process.
	query := `
	...
	// We pass the *transaction* (tx) to the DB, so our balance
	// check is part of the same atomic, isolated operation.
	// We need to create a *new* GetWalletBalance that can accept a tx.
	...
	// This is the logic from dropshipper-functions.php
