...
// It updates the quantity of a single item in the cart.
func (h *Handlers) UpdateCartItem(c *gin.Context) {
	// 1. --- Get IDs ---
	userID_raw, _ := c.Get("userID")
	dropshipperID := userID_raw.(int64)
	productIDStr := c.Param("product_id") // Get product_id from URL path
	...
	// We don't need a full transaction for this *if* we're careful.
	// But we must ensure the item belongs to the user's cart.
	var cartID int64
	err := h.DB.QueryRow("SELECT id FROM carts WHERE user_id = ?", dropshipperID).Scan(&cartID)
	if err != nil {
		if err == sql.ErrNoRows {
	...
	if input.Quantity == 0 {
		...
		// We'll call the delete logic directly.
		h.deleteCartItem(c, cartID, productIDStr)
		return
	}

	// 4. --- Check Stock ---
	// We must check if the *new* quantity is available.
	var stock int
	err = h.DB.QueryRow("SELECT stock FROM products WHERE id = ? AND status = 'published'", productIDStr).Scan(&stock)
	if err != nil {
		if err == sql.ErrNoRows {
	...
	// 5. --- Execute Update ---
	// This query atomically updates the item *only if* it exists in the user's cart.
	query := `
		UPDATE cart_items
		SET quantity = ?, updated_at = ?
		WHERE cart_id = ? AND product_id = ?`
